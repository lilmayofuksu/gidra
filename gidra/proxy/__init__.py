...
class GenshinProxy:
    ...
    def handle(self, data: bytes, direction: PacketDirection):
        ...
        try:
            ...
            # logger.debug(f'{direction.name} -> {PacketDirection(direction.value ^ 1).name}: {data.hex()}')
            self.send_raw(data, direction)
            return
        
        logger.debug(f'{direction.name} -> {PacketDirection(direction.value ^ 1).name} {packet.cmdid}:{packet.body.__class__.__name__}')
